...
Since strings and data references are both surrounded by quotes to distinguish between them data references have a “$” at the beginning of the label.  Likewise, to reserve keywords to be referenced within a specific operation (i.e. item in Apply) use "&" in front of the keyword (.i.e. "&item").
...
* __flatten__: reduce multidimensional lists to single dimension
    ...
    * OutputType - [array]
* __addCol2Table__: add a new column to a matrix from a given array
    * Input
...
* __removeStop__: given text and list of (stop) words - removes those words from text.
    * Input:
        * str - [string] The string to have stop words removed from.
    * Params:
        * lang - [string] The language to be used, based on ISO 639-1 codes https://en.wikipedia.org/wiki/List_of_ISO_639-1_codes .
            * Optional = True
            * default = "en"
        * stopwords - [array of strings] - Allows for customization of stopwords
            * Optional = True
            * default = "en"
    * OutputType - [string]
...
* __img2tensor__: Converts an image type to a mathematical tensor (i.e. an array of any number of dimentions)
    * Input
        * image - [image]
    * OutputType - [array of array or arrays (3D matrix)]
